import crypto from 'node:crypto';
import fsSync from 'node:fs';
import fs from 'node:fs/promises';
import path from 'node:path';

function syncPath(targetPath) {
  const fd = fsSync.openSync(targetPath, 'r');
  try {
    fsSync.fsyncSync(fd);
  } finally {
    fsSync.closeSync(fd);
  }
}

class CheckpointSequenceRegistry {
  constructor() {
    this.sequences = new Map();
  }

  advance(userId) {
    const sequence = (this.sequences.get(userId) || 0) + 1;
    this.sequences.set(userId, sequence);
    return sequence;
  }

  current(userId) {
    return this.sequences.get(userId);
  }

  forgetIfCurrent(userId, sequence, hasActiveSession) {
    if (hasActiveSession || this.sequences.get(userId) !== sequence) return false;
    return this.sequences.delete(userId);
  }

  get size() {
    return this.sequences.size;
  }
}

function getUserPersistencePaths(profileDir, userId) {
  const rootDir = path.resolve(profileDir);
  const safeUserDir = crypto
    .createHash('sha256')
    .update(String(userId))
    .digest('hex')
    .slice(0, 32);

  const userDir = path.join(rootDir, safeUserDir);
  return {
    rootDir,
    userDir,
    storageStatePath: path.join(userDir, 'storage-state.json'),
    metaPath: path.join(userDir, 'meta.json'),
  };
}

async function loadPersistedStorageState(profileDir, userId, logger = console) {
  if (!profileDir) return undefined;

  const { storageStatePath } = getUserPersistencePaths(profileDir, userId);

  try {
    const raw = await fs.readFile(storageStatePath, 'utf8');
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== 'object') return undefined;
    if (!Array.isArray(parsed.cookies)) return undefined;
    if (parsed.origins !== undefined && !Array.isArray(parsed.origins)) return undefined;
    return storageStatePath;
  } catch (err) {
    if (err?.code === 'ENOENT') return undefined;
    logger?.warn?.('failed to load persisted storage state', {
      userId: String(userId),
      storageStatePath,
      error: err?.message || String(err),
    });
    return undefined;
  }
}

async function persistStorageState({
  profileDir,
  userId,
  context,
  logger = console,
  shouldPublish = () => true,
  syncFile = syncPath,
  publishFile = fsSync.renameSync,
  syncDirectory = syncPath,
}) {
  if (!profileDir || !context) {
    return { persisted: false, reason: 'disabled' };
  }

  const { userDir, storageStatePath, metaPath } = getUserPersistencePaths(profileDir, userId);
  const suffix = `.tmp-${process.pid}-${Date.now()}-${crypto.randomUUID()}`;
  const tmpStoragePath = `${storageStatePath}${suffix}`;
  const tmpMetaPath = `${metaPath}${suffix}`;

  try {
    await fs.mkdir(userDir, { recursive: true });
    await context.storageState({ path: tmpStoragePath });
    await fs.writeFile(
      tmpMetaPath,
      JSON.stringify(
        {
          userId: String(userId),
          updatedAt: new Date().toISOString(),
          storageStatePath,
        },
        null,
        2
      )
    );
    syncFile(tmpStoragePath);
    syncFile(tmpMetaPath);
    if (!shouldPublish()) {
      await fs.unlink(tmpStoragePath).catch(() => {});
      await fs.unlink(tmpMetaPath).catch(() => {});
      return { persisted: false, reason: 'superseded' };
    }
    // Keep the final generation check and authoritative publication in one
    // event-loop turn so a replacement session cannot interleave between them.
    publishFile(tmpStoragePath, storageStatePath);
    let metaPersisted = true;
    try {
      // Metadata is diagnostic only; storage-state.json is authoritative.
      publishFile(tmpMetaPath, metaPath);
    } catch (metaError) {
      metaPersisted = false;
      await fs.unlink(tmpMetaPath).catch(() => {});
      logger?.warn?.('failed to persist storage state metadata', {
        userId: String(userId),
        metaPath,
        error: metaError?.message || String(metaError),
      });
    }
    // Make both rename directory entries durable before reporting success.
    syncDirectory(userDir);
    return { persisted: true, metaPersisted, userDir, storageStatePath, metaPath };
  } catch (err) {
    await fs.unlink(tmpStoragePath).catch(() => {});
    await fs.unlink(tmpMetaPath).catch(() => {});
    logger?.warn?.('failed to persist storage state', {
      userId: String(userId),
      storageStatePath,
      error: err?.message || String(err),
    });
    return { persisted: false, reason: 'error', error: err };
  }
}

export {
  CheckpointSequenceRegistry,
  getUserPersistencePaths,
  loadPersistedStorageState,
  persistStorageState,
};
