...
async function persistStorageState({
  ...
  try {
    ...
    if (!shouldPublish()) {
      await Promise.all([
        fs.unlink(tmpStoragePath).catch(() => {}),
        fs.unlink(tmpMetaPath).catch(() => {}),
      ]);
      return { persisted: false, reason: 'superseded' };
    }
    // Keep the final generation check and publication in one synchronous
    // event-loop turn. Both source files are complete before either rename.
