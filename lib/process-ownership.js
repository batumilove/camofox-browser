...
/**
 * Snapshot only descendant process trees rooted at an exact executable argv0.
 * This attributes a launch without relying on Playwright's non-public process()
 * API and without claiming unrelated children created during the same window.
 */
export function snapshotOwnedProcessTreesByExecutable(rootPid, executablePath, procRoot = '/proc') {
  if (!executablePath) return [];
  const processes = snapshotOwnedProcessDescendants(rootPid, procRoot);
  const roots = processes.filter(proc => proc.cmdline.split('\0', 1)[0] === String(executablePath));
