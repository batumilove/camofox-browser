...
/** Snapshot every descendant owned by one server process. */
export function snapshotOwnedProcessDescendants(rootPid, procRoot = '/proc') {
  ...
  return processes.filter(proc => proc.pid !== Number(rootPid) && descendants.has(proc.pid));
}

/**
 * Snapshot the one newly launched descendant process tree rooted at an exact
 * executable argv0. If more than one new root is possible, attribution is
 * ambiguous and no process is claimed.
 */
export function snapshotOwnedProcessTreesByExecutable(
  rootPid,
  executablePath,
  procRoot = '/proc',
  ...
  const processes = snapshotOwnedProcessDescendants(rootPid, procRoot);
  const byPid = new Map(processes.map(proc => [proc.pid, proc]));
  const isMatchingExecutable = proc => proc.cmdline.split('\0', 1)[0] === String(executablePath);
  const roots = processes.filter(proc => {
    if (!isMatchingExecutable(proc)) return false;
    if (excludedIdentities.has(`${proc.pid}:${proc.startTime}`)) return false;
    let ancestor = byPid.get(proc.ppid);
    while (ancestor) {
      if (isMatchingExecutable(ancestor)) return false;
  ...
  const owned = new Set(roots.map(proc => proc.pid));
  let grew = true;
  while (grew) {
    grew = false;
    for (const proc of processes) {
...
export function snapshotOwnedBrowserProcesses(rootPid, procRoot = '/proc', ownedRootPid = null) {
  ...
  if (ownedRootPid !== null && ownedRootPid !== undefined) {
    ...
    if (!processes.some(proc => proc.pid === ownedRoot)) return [];
  ...
  return processes.filter(proc => BROWSER_PROCESS_RE.test(proc.cmdline));
}
...
/** Preserve exact surviving identities across reparenting and add descendants. */
export function refreshOwnedProcessSnapshot(snapshot, procRoot = '/proc') {
  if (process.platform !== 'linux' && procRoot === '/proc') return snapshot;
  const refreshed = new Map();
  for (const identity of survivingOwnedBrowserProcesses(snapshot, procRoot)) {
    refreshed.set(`${identity.pid}:${identity.startTime}`, identity);
    for (const proc of snapshotOwnedProcessDescendants(identity.pid, procRoot)) {
      refreshed.set(`${proc.pid}:${proc.startTime}`, proc);
    }
  }
...
/** TERM an exact process generation, then escalate safely if it survives. */
export function terminateOwnedProcess(identity, {
  graceMs = 1000,
  signalOwned = signalOwnedProcess,
  setTimer = setTimeout,
} = {}) {
  if (!identity || !signalOwned(identity, 'SIGTERM')) return false;
  const timer = setTimer(() => signalOwned(identity, 'SIGKILL'), graceMs);
  timer?.unref?.();
  return true;
}
