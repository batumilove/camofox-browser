...
function settleWithin(promise, timeoutMs) {
  const observed = Promise.resolve(promise)
    .then(value => ({ settled: true, value }), error => ({ settled: true, error }));
...
export async function createOwnedResource({
  ...
  const settlement = raw.then(async (value) => {
    ...
    if (abandoned && !adopted) {
      try {
        const cleanupResult = await cleanupOnce(value);
        if (cleanupResult !== false) settleLease();
      } catch {
        // Failed cleanup retains the raw-creation lease fail closed.
      }
    }
    return value;
  }, (error) => {
  ...
  try {
    ...
    await prepare(value);
    if (signal?.aborted) throw asAbortError(signal);
    // The unresolved-creation lease ends only after lifetime ownership, when
    // requested, has been acquired successfully.
    settleLease();
    ...
    abandoned = true;
    if (resource && !adopted) {
      const cleanup = cleanupOnce(resource);
      cleanup.then(result => {
        if (result !== false) settleLease();
      }, () => {
        // Failed cleanup retains the raw-creation lease fail closed.
      });
      await settleWithin(cleanup, cleanupTimeoutMs);
    }
    if (!resource) {
...
export async function withTemporaryResource(options, operation) {
  ...
  return createOwnedResource({
    ...
    cleanup: async resource => {
      ...
      cleaned = true;
      const result = await cleanup(resource);
      if (result !== false) settleLifetime();
      return result;
    },
    prepare: async resource => {
      await prepare(resource);
      lifetimeLease = options.acquireLifetime?.(resource) || null;
    },
