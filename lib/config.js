...
function loadConfig({ configPath = CONFIG_PATH } = {}) {
  ...
  const tabAdmissionQueueLimit = parsePositiveInt(process.env.TAB_ADMISSION_QUEUE_LIMIT, 8);
  const tabAdmissionQueueLimitPerUser = Math.min(
    tabAdmissionQueueLimit,
    parsePositiveInt(
      process.env.TAB_ADMISSION_QUEUE_LIMIT_PER_USER,
      Math.max(1, Math.ceil(tabAdmissionQueueLimit / 2)),
    ),
  );
  return {
    ...
    serverEnv: {
      ...
      TAB_ADMISSION_QUEUE_LIMIT_PER_USER: process.env.TAB_ADMISSION_QUEUE_LIMIT_PER_USER,
