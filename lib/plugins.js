 ...
 *     session:destroying      { userId, reason, session, context } -- before context close (context still alive)
 *     session:destroyed       { userId, reason, session, context } -- after cleanup
 ...
 *
 *   COOKIES / AUTH
 *     session:cookies:import  { userId, count, session, context }
...
export function createPluginEvents() {
  ...
  events.onListenerError = () => {};

  function reportListenerError(error, eventName) {
    try { events.onListenerError(error, eventName); } catch (_) {}
  }

  // Observational hooks are fire-and-forget, but each listener must still be
  // isolated: one broken plugin cannot prevent sibling plugins from running
  // or create an unhandled rejected promise.
  events.emit = function emit(eventName, ...args) {
    const listeners = this.rawListeners(eventName);
    for (const listener of listeners) {
      try {
        const result = listener.apply(this, args);
        if (result && typeof result.then === 'function') {
          Promise.resolve(result).catch((error) => reportListenerError(error, eventName));
        }
      } catch (error) {
        reportListenerError(error, eventName);
      }
    }
  ...
  events.emitAsync = async function emitAsync(eventName, payload) {
    const listeners = this.rawListeners(eventName);
    await Promise.all(listeners.map(fn => Promise.resolve().then(() => fn.call(this, payload))));
  };

  /**
   * Await every listener without allowing one rejection to interrupt a
   * lifecycle teardown. The optional callback receives each rejection.
   */
  events.emitAsyncSettled = async function emitAsyncSettled(eventName, payload, onRejected) {
    const listeners = this.rawListeners(eventName);
    const results = await Promise.allSettled(
      listeners.map(fn => Promise.resolve().then(() => fn.call(this, payload)).catch((error) => {
        if (onRejected) {
          try { onRejected(error); } catch (_) {}
        }
        throw error;
      })),
    );
