...
export function createPluginEvents() {
  ...
  events.emitAsync = async function emitAsync(eventName, payload) {
    ...
    // Wait for EVERY listener to settle, even if some fail, so callers that
    // clean up on error never race still-running mutating hooks.
    const outcomes = await Promise.allSettled(listeners.map(fn => fn(payload)));
