 ...
 *     session:destroying      { userId, context, reason }      -- before context close (context still alive)
 *     session:destroyed       { userId, context, reason }      -- after cleanup
 ...
 *     session:storage:export  { userId, context, storageState }
 *
 *   SERVER
...
export function createPluginEvents() {
  ...
  events.emitAsync = async function emitAsync(eventName, payload) {
    ...
    // Invoke every listener, even if one throws synchronously, and wait for
    // ALL of them to settle so callers that clean up on error never race
    // still-running mutating hooks.
