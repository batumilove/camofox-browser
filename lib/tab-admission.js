...
export function settleAllConcurrently(items, operation) {
  return Promise.allSettled(items.map((item) => operation(item)));
}
...
export async function awaitAbortableResource(resourcePromise, signal, cleanup) {
  const pending = Promise.resolve(resourcePromise);
  if (!signal) return pending;
  return new Promise((resolve, reject) => {
    let settled = false;
    const abortError = () => signal.reason instanceof Error ? signal.reason : new Error('Operation aborted');
    const cleanupLate = (resource) => Promise.resolve(cleanup(resource)).catch(() => {});
    const onAbort = () => {
      if (settled) return;
      settled = true;
      signal.removeEventListener('abort', onAbort);
      pending.then(cleanupLate, () => {});
      reject(abortError());
    };
