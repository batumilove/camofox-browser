...
export class OrphanPageCleanup {
  ...
  async cleanup(session, page, reason = 'orphan_cleanup') {
    if (!page || page.isClosed?.()) {
      if (page) this.attempts.delete(page);
      return true;
    }
    ...
    try {
      await Promise.resolve(this.closePage(page, session, reason)).catch(() => {});
      if (page.isClosed?.()) {
        this.attempts.delete(page);
        return true;
      }

      const attempts = (this.attempts.get(page) || 0) + 1;
      this.attempts.set(page, attempts);
      if (attempts >= this.maxAttempts && session && !this.escalatingSessions.has(session)) {
        this.escalatingSessions.add(session);
        await Promise.resolve(this.onEscalate(session, page, reason, attempts)).catch(() => {});
      }
      return false;
    } finally {
...
export async function withAbortableResource({
  ...
  try {
    ...
    resource = await awaitAbortableResource(creation, signal, cleanupResource, {
