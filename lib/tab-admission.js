...
export class RawCreationRegistry {
  constructor({
    ...
    this.deadlineRetryMs = positiveInteger(deadlineRetryMs, 5000);
    this.onRejected = onRejected;
    this.onDeadline = onDeadline;
    this.onDeadlineError = onDeadlineError;
    this.entries = new Map();
    this.byUser = new Map();
  }

  acquire({ userKey, kind = 'page', owner = null, deadlineMs, onDeadline, onRetire = () => {} } = {}) {
    ...
    const settle = ({ retired = false } = {}) => {
      ...
      if (retired) {
        try { onRetire(entry); } catch (error) {
          try { this.onDeadlineError(error, entry); } catch { /* observer must not break retirement */ }
        }
      }
      return true;
    };
    ...
    if (boundedDeadlineMs > 0) {
      ...
      const invokeDeadline = () => {
        if (!this.entries.has(id) || entry.retired) return;
        Promise.resolve()
          .then(() => handler(entry))
          .catch(error => {
            if (!this.entries.has(id) || entry.retired) return;
            try { this.onDeadlineError(error, entry); } catch { /* observer must not break retry */ }
            entry.timer = setTimeout(invokeDeadline, this.deadlineRetryMs);
            entry.timer.unref?.();
          });
      };
    ...
    const retire = () => settle({ retired: true });
    entry.retire = retire;
    return { id, settle, retire, entry };
  }
...
export class TabAdmissionController {
  constructor({
    ...
    this.maxPendingPerUser = positiveInteger(maxPendingPerUser, this.maxPending);
  ...
  snapshot() {
    return {
      ...
      pendingByUser: Object.fromEntries(this.pendingByUser),
    };
  }
  ...
  run(userKey, operation, { signal } = {}) {
    ...
    const userPending = this.pendingByUser.get(key) || 0;
    if (!canStartNow && userPending >= this.maxPendingPerUser) {
      this.onRejected();
      return Promise.reject(new TabAdmissionError('Tab admission queue is full for this user', {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.pending += 1;
      this.pendingByUser.set(key, userPending + 1);
  ...
  #abortEntry(entry) {
    ...
    if (entry.queued) {
      ...
      entry.queued = false;
      this.pending -= 1;
      this.#decrementPending(entry.key);
  ...
  #decrementPending(key) {
    const remaining = (this.pendingByUser.get(key) || 1) - 1;
    if (remaining > 0) this.pendingByUser.set(key, remaining);
    else this.pendingByUser.delete(key);
  }
}
...
export function popupOwnerIsCurrent({
  currentSession,
  ownerSession,
  popupContext,
  ownerContext,
  ...
  return Boolean(
    ...
    && !ownerSession?._closing
    && ownerSession?.browserGeneration === ownerGeneration
...
export class OrphanPageCleanup {
  ...
  async #escalate(entry) {
    ...
    const timeout = new Promise(resolve => {
      timer = setTimeout(() => resolve(null), this.escalationTimeoutMs);
      timer.unref?.();
    });
    try {
