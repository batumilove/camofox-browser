...
export class SessionCreationGenerations {
  ...
  begin(userKey) {
    const key = String(userKey);
    const token = Object.freeze({ key, globalGeneration: this.globalGeneration });
    this.activeByUser.set(key, token);
    return token;
  }

  canPublish(token) {
    return token?.globalGeneration === this.globalGeneration
      && this.activeByUser.get(token.key) === token;
  }

  invalidate(userKey) {
    this.activeByUser.delete(String(userKey));
  }
...
export async function closePageWithin(page, { timeoutMs, onFailure = () => {} } = {}) {
...
export async function runBoundedSessionTeardown({
  ...
  const settleOperation = async (name, operation) => {
    const result = await settleWithin(Promise.resolve().then(operation), timeoutMs);
