...
function nonNegativeInteger(value) {
  ...
  return Math.min(Math.trunc(value), MAX_DIAGNOSTIC_COUNTER);
}
...
export function collectUserDiagnostics({
  ...
  if (currentSession) {
    for (const [sessionKey, group] of currentSession.tabGroups) {
      for (const [tabId, state] of group) {
        ...
        const lockQueued = nonNegativeInteger(Array.isArray(lock?.queue) ? lock.queue.length : 0);
        if (lockActive) activeLocks = nonNegativeInteger(activeLocks + 1);
        queuedLocks = nonNegativeInteger(queuedLocks + lockQueued);
        tabs.push({
          tabId: String(tabId),
  ...
  return {
    ...
    session: {
      ...
      lastAccess,
      idleMs: lastAccess === null ? null : nonNegativeInteger(now - lastAccess),
      pendingTabCreations: nonNegativeInteger(currentSession?._pendingTabCreations),
      tabCount: nonNegativeInteger(tabs.length),
    ...
    concurrency: {
      ...
      queuedForUser: nonNegativeInteger(Array.isArray(concurrency?.queue) ? concurrency.queue.length : 0),
    },
    locks: {
