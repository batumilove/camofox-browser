...
export async function register(app, ctx, pluginConfig = {}) {
  ...
  events.on('session:destroying', async ({ userId, reason, context }) => {
    const closingContext = context || activeSessions.get(userId);
    if (closingContext && reason !== 'storage_reset') {
      await checkpoint(userId, closingContext, reason).catch(() => {});
    }
  ...
  // On session destroyed (post-close): cleanup only the context that closed.
