...
async function createUnregisteredManagedPage(userId, initialSession, { trace = false, signal } = {}) {
  ...
  try {
    ...
    if (createdPage) await safePageClose(createdPage);
    throw error;
  } finally {
    capacityLease();
    pendingLease();
...
function handleRouteError(err, req, res, extraFields = {}) {
  ...
  if (isTimeoutError(err) && userId && foundSession && NAVIGATION_TIMEOUT_ACTIONS.has(action)) {
  ...
  try {
    ...
    const result = await withTabAdmission(userId, async (signal) => {
      ...
      try {
        ...
        if (!((isProxyError(navErr) || isTimeoutError(navErr)) && proxyPool?.canRotateSessions)) throw navErr;
